...
data class RemoveMark(val mark: Mark) : FunctionCommand {
    override fun execute(controller: IController) {
        val value = controller.getValue()

        if (value.selection.isExpanded()) {
            controller.command(RemoveMarkAtRange(value.selection, mark))
        } else {
            ...
            controller.command(
                Select(
                    value.selection.copy(marks = marks.minus(mark))
                )
            )
        }
    }
